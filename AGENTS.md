...
assertions in the module tree, so they fire on `nix eval` of a host rather than
on `nix flake check`. The parse checks scan broadly, not per-directory: every
`.zsh` and `.lua` under `modules/`, and every shell script in the whole flake
source, selected by shebang as well as by extension. Each also asserts that
