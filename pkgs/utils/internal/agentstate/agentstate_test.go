...
func TestDeriveReasonPerSource(t *testing.T) {
	...
	for _, tc := range cases {
		got := deriveReason(tc.src, "working", tc.input, claudeKeys, dir, "1", 100)
		if got != tc.want {
...
func TestSubmitWritesTheStartStamp(t *testing.T) {
	dir := t.TempDir()
	if got := deriveReason("submit", "working", nil, claudeKeys, dir, "7", 1234); got != "thinking" {
...
func TestReapRemovesOnlyRecordsFromADeadMux(t *testing.T) {
	...
	for _, pane := range []string{"current", "live"} {
	...
	if _, err := os.Stat(filepath.Join(dir, "unmarked.json")); !os.IsNotExist(err) {
		t.Errorf("unmarked.json survived the reap: %v", err)
	}
