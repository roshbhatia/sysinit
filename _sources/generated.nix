# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  crush = {
    pname = "crush";
    version = "0.79.1";
    src = fetchurl {
      url = "https://github.com/charmbracelet/crush/releases/download/v0.79.1/crush_0.79.1_Darwin_arm64.tar.gz";
      sha256 = "sha256-hQp/r1WProRHGb+AdrFrECjOAUAIs4GwfVa1m5m6qzc=";
    };
  };
  crush-aarch64-linux = {
    pname = "crush-aarch64-linux";
    version = "0.79.1";
    src = fetchurl {
      url = "https://github.com/charmbracelet/crush/releases/download/v0.79.1/crush_0.79.1_Linux_arm64.tar.gz";
      sha256 = "sha256-yGAp9KAtC8YIDAAXnKSpYQyYvz84aiOs+QMYM6R/3sE=";
    };
  };
  crush-x86_64-darwin = {
    pname = "crush-x86_64-darwin";
    version = "0.79.1";
    src = fetchurl {
      url = "https://github.com/charmbracelet/crush/releases/download/v0.79.1/crush_0.79.1_Darwin_x86_64.tar.gz";
      sha256 = "sha256-8AyC8B97Jwn6869TvQCiIGySRMibPgs4s01sYqeNUc0=";
    };
  };
  crush-x86_64-linux = {
    pname = "crush-x86_64-linux";
    version = "0.79.1";
    src = fetchurl {
      url = "https://github.com/charmbracelet/crush/releases/download/v0.79.1/crush_0.79.1_Linux_x86_64.tar.gz";
      sha256 = "sha256-wUTvCUETd5zbOnqcMnrpXuwvgwHntHrkFiTXXmyx6Dg=";
    };
  };
  go-enum = {
    pname = "go-enum";
    version = "v0.9.2";
    src = fetchFromGitHub {
      owner = "abice";
      repo = "go-enum";
      rev = "v0.9.2";
      fetchSubmodules = false;
      sha256 = "sha256-VZH7xLEDqU8N7oU3tOWVdTABEQEp2mlh1NtTg22hzco=";
    };
  };
  gomvp = {
    pname = "gomvp";
    version = "v0.0.4";
    src = fetchFromGitHub {
      owner = "abenz1267";
      repo = "gomvp";
      rev = "v0.0.4";
      fetchSubmodules = false;
      sha256 = "sha256-dXjI+nItJCAGKxyC9tX11hxWHCP+NgXtTYtm5+6dqDU=";
    };
  };
  goose-cli-bin = {
    pname = "goose-cli-bin";
    version = "1.38.0";
    src = fetchurl {
      url = "https://github.com/block/goose/releases/download/v1.38.0/goose-aarch64-apple-darwin.tar.gz";
      sha256 = "sha256-PGt88yFhDdiiAwfKdqTHVgj7X+Ay1Pn9357hx2Hx1Kk=";
    };
  };
  goose-cli-bin-aarch64-linux = {
    pname = "goose-cli-bin-aarch64-linux";
    version = "1.38.0";
    src = fetchurl {
      url = "https://github.com/block/goose/releases/download/v1.38.0/goose-aarch64-unknown-linux-gnu.tar.gz";
      sha256 = "sha256-YBJCrTP7vXCzpyGBqh1EkJb2m2CBUhJO8WdXAZ3sPsA=";
    };
  };
  goose-cli-bin-x86_64-darwin = {
    pname = "goose-cli-bin-x86_64-darwin";
    version = "1.38.0";
    src = fetchurl {
      url = "https://github.com/block/goose/releases/download/v1.38.0/goose-x86_64-apple-darwin.tar.gz";
      sha256 = "sha256-EjNkpD0pWy8z4kXUhp7G/h5CmRruieXrQe/uAw51eN4=";
    };
  };
  goose-cli-bin-x86_64-linux = {
    pname = "goose-cli-bin-x86_64-linux";
    version = "1.38.0";
    src = fetchurl {
      url = "https://github.com/block/goose/releases/download/v1.38.0/goose-x86_64-unknown-linux-gnu.tar.gz";
      sha256 = "sha256-cFMuos59OEYctnDmlvork2dNFhdew5PyqVTvYkFh/50=";
    };
  };
  hererocks = {
    pname = "hererocks";
    version = "0.25.1";
    src = fetchFromGitHub {
      owner = "luarocks";
      repo = "hererocks";
      rev = "0.25.1";
      fetchSubmodules = false;
      sha256 = "sha256-y28MTFncy5oD57jpY6AN+X/58OzY3ae3rSL236rfuL0=";
    };
  };
  ioskeleyMono = {
    pname = "ioskeleyMono";
    version = "2026.03.19-7";
    src = fetchurl {
      url = "https://github.com/ahatem/IoskeleyMono/releases/download/2026.03.19-7/IoskeleyMono-TTF-Hinted.zip";
      sha256 = "sha256-641PVxt0MVXQentV5oWObBpu2P0e3qtfpWmbdOuNUtI=";
    };
  };
  kubernetes-zeitgeist = {
    pname = "kubernetes-zeitgeist";
    version = "v0.7.0";
    src = fetchFromGitHub {
      owner = "kubernetes-sigs";
      repo = "zeitgeist";
      rev = "v0.7.0";
      fetchSubmodules = false;
      sha256 = "sha256-tzR9hOMfnEa4p1ssktTISO6cAwlSxEGfvzJeLyM2qYo=";
    };
  };
  mermaid-ascii = {
    pname = "mermaid-ascii";
    version = "1.2.0";
    src = fetchFromGitHub {
      owner = "AlexanderGrooff";
      repo = "mermaid-ascii";
      rev = "1.2.0";
      fetchSubmodules = false;
      sha256 = "sha256-PbYqYy0jHQb+qvkp+lp/Wlde50dhdg7Ri2eNExXeoQ4=";
    };
  };
  pi-coding-agent = {
    pname = "pi-coding-agent";
    version = "0.79.9";
    src = fetchurl {
      url = "https://github.com/earendil-works/pi/releases/download/v0.79.9/pi-darwin-arm64.tar.gz";
      sha256 = "sha256-ljK/MMiah+LcZ04kUBVoBhhzEdKTsP/7wVB3rFHkbUg=";
    };
  };
  pi-coding-agent-aarch64-linux = {
    pname = "pi-coding-agent-aarch64-linux";
    version = "0.79.9";
    src = fetchurl {
      url = "https://github.com/earendil-works/pi/releases/download/v0.79.9/pi-linux-arm64.tar.gz";
      sha256 = "sha256-GWR7hTDGhswqZcGg35Ox6LQRGJsNyNXuWo/6RHpc3W4=";
    };
  };
  pi-coding-agent-x86_64-darwin = {
    pname = "pi-coding-agent-x86_64-darwin";
    version = "0.79.9";
    src = fetchurl {
      url = "https://github.com/earendil-works/pi/releases/download/v0.79.9/pi-darwin-x64.tar.gz";
      sha256 = "sha256-TNw5kLgJGFTJf/SheSt4yaIwPmCQ32dgtSUgCNY9Yfk=";
    };
  };
  pi-coding-agent-x86_64-linux = {
    pname = "pi-coding-agent-x86_64-linux";
    version = "0.79.9";
    src = fetchurl {
      url = "https://github.com/earendil-works/pi/releases/download/v0.79.9/pi-linux-x64.tar.gz";
      sha256 = "sha256-1N8tQbmlDRz8HsO/dQ1TJWRhusbWLs5DG4IIwVBPyJQ=";
    };
  };
}
