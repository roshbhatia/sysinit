# Sunshine/Moonlight game-streaming stack for arrakis.
#
# Sunshine (system service, modules/nixos/desktop/sway.nix) captures the
# desktop via KMS and encodes with NVENC. These scripts are the launch layer
# Moonlight tiles call into: each game runs inside a nested gamescope sized
# to the client-requested resolution, and the physical output is switched to
# match for a zero-scaling capture path.
#
# apps.json is generated at runtime by sunshine-apps-gen (mutable state, not
# nix-managed); rerun it after installing or removing games.
{ pkgs, ... }:

let
  # Sunshine runs with CAP_SYS_ADMIN for KMS capture. File capabilities
  # trigger AT_SECURE, and inherited ambient caps make bubblewrap (steam's
  # sandbox, pressure-vessel) refuse to start — every launcher drops caps
  # before exec.
  ...
  heroic-run-game = pkgs.writeShellApplication {
    ...
    runtimeInputs = [ pkgs.util-linux ];
    text = ''
      # Usage: heroic-run-game <runner> <app-name>   (runner: legendary | gog)
      RUNNER=$1
      APP=$2
...
in
{
  ...
  # apps.json is generated by sunshine-apps-gen and stays mutable; only the
  # static daemon config is declared here.
