...
async function treeState(pi: ExtensionAPI, cwd: string): Promise<TreeState> {
  ...
  const hasHead = await pi.exec("git", [
    "-C",
    cwd,
    "rev-parse",
    "--verify",
  ...
  if (hasHead.code !== 0) {
    const staged = await pi.exec("git", [
      "-C",
      cwd,
      "diff",
      "--cached",
    ...
    return staged.code === 0 && staged.stdout.trim().length > 0
      ? "dirty"
      : "clean";
  }
  const tracked = await pi.exec("git", [
    "-C",
    cwd,
    "diff",
    "HEAD",
...
async function open(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
  ...
  if (!split) {
    notify(
      ctx,
      `diff review: no WezTerm pane. Run: ${VIEWER_COMMAND.join(" ")}`,
      "warning",
    );
    return;
  }
  ...
  if (result.code !== 0) {
    notify(
      ctx,
      `diff review: split failed, ${result.stderr.trim() || `exit ${result.code}`}`,
      "warning",
    );
    return;
  }
  notify(
    ctx,
    "diff review: opened in review. Re-run `review` to read the notes.",
  );
