...
interface WorkspaceData { isRepo: boolean; root: string | null; files: WorkspaceFile[]; }
...
// 49 (default background) matters as much as 39 here: `pad` appends spaces after a
// styled run, and without it those spaces inherit the run's background.
function reset(): string { return "\x1b[22;23;24;39;49m"; }
...
// Every painted row must occupy the full sidebar width.
...
// OSC 8, so WezTerm makes a changed file ctrl+clickable.
...
function summaryHeader(theme: Theme, title: string, summary: string, width: number): string[] { const left = ` ${title}`; const gap = Math.max(1, width - visibleWidth(left) - visibleWidth(summary) - 1); return [theme.bold(left) + " ".repeat(gap) + dim(theme, summary), dim(theme, "─".repeat(width))]; }
...
// active one.
function pickActive(changes: OpenSpecChange[]): OpenSpecChange { return changes.reduce((best, entry) => entry.lastModified > best.lastModified ? entry : best); }
async function readState(pi: ExtensionAPI): Promise<SidebarState> { const listed = await withTimeout(pi.exec("openspec", ["list", "--json", "--no-color"])); if (!listed || listed.code !== 0) return { active: null, otherCount: 0, unavailable: true }; const changes = parseChanges(listed.stdout); if (!changes) return { active: null, otherCount: 0, unavailable: true }; if (changes.length === 0) return { active: null, otherCount: 0, unavailable: false }; const chosen = pickActive(changes); const status = await withTimeout(pi.exec("openspec", ["status", "--change", chosen.name, "--json", "--no-color"])); const active = { ...chosen, artifacts: status && status.code === 0 ? parseArtifacts(status.stdout) : [] }; return { active, otherCount: changes.length - 1, unavailable: false }; }
async function refresh(pi: ExtensionAPI): Promise<SidebarState> { if (cache && Date.now() - cache.at < CACHE_TTL_MS) return cache.state; const state = await readState(pi); cache = { state, at: Date.now() }; return state; }
...
// `rev-parse --git-dir`, not `branch --show-current`, is the repo test: a detached HEAD
// has no current branch and read as "not a git repo".
function getWorkspace(cwd: string | undefined): WorkspaceData { const empty: WorkspaceData = { isRepo: false, root: null, files: [] }; if (!cwd) return empty; if (workspaceCache && Date.now() - workspaceCache.at < 2_000) return workspaceCache.data; if (!runGit(cwd, ["rev-parse", "--git-dir"])) return empty; const root = runGit(cwd, ["rev-parse", "--show-toplevel"]).trim() || null; const files = runGit(cwd, ["diff", "--numstat", "HEAD"]).split("\n").flatMap((line) => { const [added, removed, path] = line.split("\t"); const plus = Number.parseInt(added ?? "", 10); const minus = Number.parseInt(removed ?? "", 10); return Number.isFinite(plus) && Number.isFinite(minus) && path ? [{ path, added: plus, removed: minus }] : []; }).sort((left, right) => right.added + right.removed - left.added - left.removed).slice(0, 15); const data: WorkspaceData = { isRepo: true, root, files }; workspaceCache = { data, at: Date.now() }; return data; }
...
function renderWorkspace(workspace: WorkspaceData, theme: Theme, width: number): string[] { if (!workspace.isRepo) return []; const added = workspace.files.reduce((sum, file) => sum + file.added, 0); const removed = workspace.files.reduce((sum, file) => sum + file.removed, 0); const summary = workspace.files.length === 0 ? "clean" : `${workspace.files.length} file${workspace.files.length === 1 ? "" : "s"}  +${added} -${removed}`; const lines = summaryHeader(theme, "Changes", summary, width); if (workspace.files.length === 0) return lines; for (const file of workspace.files) { const stat = `+${file.added}${file.removed > 0 ? ` -${file.removed}` : ""}`; const label = truncate(file.path, Math.max(1, width - stat.length - 2)); lines.push(" " + (workspace.root ? link(fileUri(workspace.root, file.path), label) : label) + " " + style(theme, "success", stat)); } return lines; }
function renderSidebar(state: SidebarState, session: SessionData, subagents: SubagentEntry[], cwd: string | undefined, theme: Theme, width: number): string[] { return [renderSession(session, theme, width), renderOpenSpec(state, theme, width), renderSubagents(subagents, theme, width), renderWorkspace(getWorkspace(cwd), theme, width)].filter((panel) => panel.length > 0).flatMap((panel, index) => index === 0 ? panel : ["", ...panel]); }
