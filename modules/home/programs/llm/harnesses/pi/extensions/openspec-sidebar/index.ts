...
// 49 (default background) matters as much as 39 here: `pad` appends spaces after
// a styled run, and without it those spaces inherit the run's background.
...
// active one. Ties resolve to the first, deterministically.
...
// `rev-parse --git-dir`, not `branch --show-current`, is the repo test: a detached
// HEAD has no current branch and read as "not a git repo".
...
export default function (pi: ExtensionAPI): void {
  ...
  // Shared by the command's bare form and the keybinding, so the two can never
