...
def snapshot(source, target, codesign):
    ...
    for name in names:
        ...
        result[name] = {"source": str(origin.resolve(strict=True)),
...
def main():
    ...
    temporary = None
    try:
        previous = json.loads(args.state.read_text()) if args.operation == "check" else None
        current = snapshot(args.source, args.target, args.codesign)
        if args.operation == "check":
            return 0 if current == previous else 1
        args.state.parent.mkdir(parents=True, exist_ok=True)
        with tempfile.NamedTemporaryFile(mode="w", dir=args.state.parent, delete=False) as handle:
