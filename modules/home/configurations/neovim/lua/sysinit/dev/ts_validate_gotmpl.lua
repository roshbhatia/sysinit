...
local function classify(line)
  for name, pat in pairs({
    ...
    if line:match(pat) then
      if name == 'override_raw' then return 'gotmpl', name end
      if name == 'override_yaml' then return 'yaml', name end
      if name == 'override_json' then return 'json', name end
    end
  end
  if line:match(patterns.json_open_obj) or line:match(patterns.json_open_arr) or line:match(patterns.json_key)
    or line:match(patterns.json_close) or line:match(patterns.json_primitive) then
  ...
  if line:match(patterns.k8s_fallback)
    and not (line:match('{{') or line:match('}}') or line:match(patterns.json_open_obj)
      or line:match(patterns.json_open_arr) or line:match(patterns.json_key)) then
