...
# ]
# ///
"""Before read file hook - opens file in Neovim buffer before agent reads it."""
...
def detect_agent_type():
    ...
    # Try importing adapters to detect agent
    try:
        sys.path.insert(
...
def main():
    ...
    # Load configuration
    config = load_config()
    logger = create_logger(debug=config.debug, hook_name="before_read_file")
    ...
    try:
        ...
        if agent_type == "cursor":
            from cursor.adapter import parse_before_read_file, format_output
        elif agent_type == "claude":
            from claude.adapter import parse_before_read_file, format_output
        elif agent_type == "opencode":
            from opencode.adapter import parse_before_read_file, format_output
        else:
            # Generic fallback - assume first arg is file path
        ...
        if agent_type != "unknown":
            file_path = parse_before_read_file()
        else:
            file_path = sys.argv[1] if len(sys.argv) > 1 else None
        ...
        # Execute hook
        result = handlers.before_read_file(file_path)
