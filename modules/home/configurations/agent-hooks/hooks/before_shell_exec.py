...
# ]
# ///
"""Before shell execution hook - notifies Neovim about agent shell commands."""
...
def main():
    ...
    # Load configuration
    config = load_config()
    logger = create_logger(debug=config.debug, hook_name="before_shell_exec")
    ...
    try:
        ...
        if agent_type == "cursor":
            from cursor.adapter import parse_before_shell_exec, format_output
        elif agent_type == "claude":
            from claude.adapter import parse_before_shell_exec, format_output
        elif agent_type == "opencode":
            from opencode.adapter import parse_before_shell_exec, format_output
        else:
            # Generic fallback
        ...
        if agent_type != "unknown":
            parsed = parse_before_shell_exec()
            if not parsed:
        ...
        # Execute hook
        result = handlers.before_shell_exec(command, cwd)
