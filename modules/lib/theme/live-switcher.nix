...
let
  ...
  allThemeCombinations = flatten (
    mapAttrsToList (colorscheme: themeData:
      map (variant: {
...
${builtins.toJSON {
  ...
  in {
    ...
    neovim = if builtins.isAttrs rawNeovimTheme && builtins.hasAttr "colorscheme" rawNeovimTheme
...
EOF
  ...
  home.file."Library/LaunchAgents/com.sysinit.theme-manager.plist".text = ''
    ...
    <dict>
      ...
      <dict>
        <key>PATH</key>
        <string>${lib.makeBinPath [ pkgs.jq pkgs.fswatch ]}</string>
      </dict>
    </dict>
    </plist>
  '';
