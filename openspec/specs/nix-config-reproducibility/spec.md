# nix-config-reproducibility Specification

## Purpose
Guarantee that every package source resolves from a pinned lockfile or nvfetcher `_sources`, that no eval-time fetch is out of lock, and that no oversized build artifact is tracked in git.
