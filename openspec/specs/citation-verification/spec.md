...
- **THEN** `specreview` treats it as positive per the marker, so the lint verdict does not depend on prose wording, and the canonical `#### Scenario:` heading keeps openspec archive from dropping it

#### Scenario: Critique is not claimed deterministic
- **POLARITY** positive
- **WHEN** the adversarial-review skill documents the critique step
- **THEN** it states the critique is stabilized but not bit-deterministic, and only the `specreview` rubric-lint is reproducible
