...
- [ ] 1.1 Record every check's `drvPath` before any edit, so each later move has an
      exact baseline to compare against (19 checks)
- [ ] 1.2 Add `checks/default.nix` taking one attrset and returning the checks
...
- [ ] 2.1 Move `managed-file-reconcile` (274 lines) and `managed-file-merge3` (81),
      which share a subject and a prelude `deps:` 1.6
- [ ] 2.2 Move `notify-defect-regressions` (171 lines), which carries two roots
      (`$cfg` and `$harness`) and the `require_file` guards `deps:` 1.6
- [ ] 2.3 Move `destructive-guard-fixtures` (118) and `exit-code-guard-blocks`,
      which share the guard subject `deps:` 1.6
- [ ] 2.4 Move `wezterm-chord-collisions` (103) and `shell-scripts-shellcheck`
      (105). The latter must not exclude its own new siblings `deps:` 1.6
- [ ] 2.5 Move `citelock` (82), `skill-render-shape` (66), `pi-settings-keys-exist`
      (69), and `opencode-config-schema` (62) `deps:` 1.6
- [ ] 2.6 Move the remainder: `schema-templates-conform`, `zsh-fragments-parse`,
      `lua-parses`, `openspec-default-schema`, `pi-no-theme-writer`,
      `pi-shell-prefix-loads-aliases`, and `llm-asset-paths-resolve` `deps:` 1.6
- [ ] 2.7 Confirm all 19 `drvPath` values still equal the 1.1 baseline. A single
      mismatch means that check's body changed and the move is wrong
      `deps:` 2.1, 2.2, 2.3, 2.4, 2.5, 2.6
...
- [ ] 3.1 Remove the emptied checks region from `flake.nix` and confirm the file is
...
- [ ] 3.5 Format `flake.nix` and every new file with `nix fmt`, in its own commit,
