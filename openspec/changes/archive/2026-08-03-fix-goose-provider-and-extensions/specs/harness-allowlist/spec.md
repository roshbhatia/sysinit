...
The allowlist MUST expose `formatFor<Harness>` for each harness whose native config can express a bash-allowlist: `formatForClaude` (returns `["Bash(<pattern>)" ...]`), `formatForOpencode` (returns the per-tool `bash.<key> = "allow"` attrset), `formatForAmp` (returns the `tool×match×action` triples), `formatForCrush` (returns the `allowed_tools` list), `formatForCursor` (returns `["Shell(<pattern>)" ...]`).

Harnesses that do not have a native bash-allowlist concept (`codex`, `copilot-cli`, `gemini`, `goose`) SHALL NOT consume `allowlist`. Their configs MUST NOT reference it.

Goose is a non-consumer as of goose 1.28. Its `shell.allow` and `shell.deny` keys are ignored by the binary, so `formatForGoose` and `formatDestructiveForGoose` rendered config that never took effect. Both are deleted. Goose's permission surface is tool-level (`permission.yaml`) and has no command-pattern form to render into.
...
- **POLARITY** negative
- **WHEN** a harness config for `codex`, `copilot-cli`, `gemini`, or `goose` is written to consume `allowlist`
- **THEN** the config is rejected in review, because those harnesses have no native bash-allowlist concept to render it into

#### Scenario: The goose formatters are gone
- **POLARITY** negative
- **WHEN** `lib/allowlist.nix` is evaluated and its attribute names are inspected
- **THEN** neither `formatForGoose` nor `formatDestructiveForGoose` is present
