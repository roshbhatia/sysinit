...
- [x] 1.1 Add `modules/home/programs/llm/config/agent-state.sh`, modeled on
...
- [x] 1.2 Export the emitter from `notify.nix` via `pkgs.writeShellApplication`,
  following the existing `script`/`focusScript` pattern; expose its path
  (e.g. `stateExe`).
- [x] 1.3 Wire Claude hooks in `claude.nix`: add the emitter to
  ...
  and `Stop` unchanged.
- [x] 1.4 Wire Codex hooks in `codex.nix`: add the emitter to `PermissionRequest`
  (waiting) and `Stop` (done) alongside the existing notifier.
- [x] 1.5 Verify: `nix flake check` green; `nh darwin build` green.

## 2. Rollup (capability: agent-session-rollup)

- [x] 2.1 Add a shared helper (new function in `pkg/ui.lua` or a small `pkg/`
...
- [x] 2.2 Verify: `nix flake check` + `nh darwin build` green. No surface reads
  the helper yet — nothing user-visible changes.

## 3. Switcher (capability: agent-aware-switcher)

- [x] 3.1 Enrich `wm.get_choices` (`ui.lua:~513`) labels from
...
- [x] 3.2 Verify: `nh darwin build` green.
- [x] 3.3 **Human checkpoint — live spot-check.** Open `SUPER+s` with a real
...
- [x] 4.1 Rewrite `agent_status()` (`ui.lua:~196`) to name the worst session
...
- [x] 4.2 Verify: `nh darwin build` green.

## 5. Apply & confirm (impactful actions)

- [x] 5.1 Review `git diff`; confirm `nix flake check` and `nh darwin build`
  are green.
- [x] 5.2 **Impactful action — apply.** `nh darwin switch` (mutates the live
  system).
- [x] 5.3 **Confirm live.** Blocked agent → switcher row and statusline both
  name it with reason/age; a hookless agent still appears via `agent-deck`
  fallback.
- [x] 5.4 **Impactful action — publish.** Commit the implementation (conventional
  commit, title-only) and `git push` to `main` (this repo commits straight to
  main).
- [x] 5.5 Ping the user that it is applied.
