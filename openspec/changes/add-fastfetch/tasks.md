...
- [x] 1.8 Verify: `wc -l modules/home/programs/fastfetch/art/*.txt` shows non-empty files
...
- [x] 2.3 Define the hostname → art-name attrset (`lv426 → rosh`, `arrakis → nix`, `nostromo → nix`) with `or "rosh"` fallback
...
- [x] 2.8 Wire `xdg.configFile."fastfetch/art/<name>.txt".source = ./fastfetch/art/<name>.txt` for all six files
...
- [x] 4.2 Verify: `nh darwin build` succeeds on the current host (`lv426`)
- [x] 4.3 Verify: inspect the produced `home-files/.config/fastfetch/config.jsonc` in the closure store path — every `keyColor` matches `^38;2;\d{1,3};\d{1,3};\d{1,3}$`, cluster order is Hardware → Firmware → Software → Theme → Network, `logo.source = "/Users/rshnbhatia/.config/fastfetch/logo.txt"`
- [x] 4.4 Verify: `home-files/.config/fastfetch/art/` contains all six files (mgs, nix, rosh, rosh-color, vagabond, varre)
- [x] 4.5 Verify: `home-files/.config/fastfetch/logo.txt` symlinks to `hm_rosh.txt` (lv426)
...
- [ ] 6.1 Verify: on `arrakis`, pull and run `nh os build`; closure builds; `Software` cluster command is the `nix-store` variant
- [ ] 6.2 Apply: `nh os switch` on `arrakis`
- [ ] 6.3 Confirm: run `fastfetch` on `arrakis`; the `nix` art renders, package count shows a single `N (nix-store)` value
- [ ] 6.4 Confirm (optional, if user runs nostromo): `nh os switch` on `nostromo`, run `fastfetch`, verify `nix` art renders inside the lima VM

## 7. Closeout

- [ ] 7.1 Stage and propose a commit message (do not commit) in the form `feat(fastfetch): add home-manager module with macchina art`
