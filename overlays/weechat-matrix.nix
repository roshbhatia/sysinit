...
let
  ...
  drv = final.rustPlatform.buildRustPackage {
    ...
    postInstall = ''
      ...
      cp $out/lib/libmatrix${final.stdenv.hostPlatform.extensions.sharedLibrary} $out/lib/weechat/plugins/matrix.so
    '';
...
in
{
  weechatScripts = prev.weechatScripts // {
    weechat-matrix-rs = drv // {
      pluginFile = "${drv}/lib/weechat/plugins/matrix.so";
    };
  };
}
