...
def update():
    tracked = subprocess.check_output(
        ["git", "ls-files", "--", "flake.nix", "*/flake.nix"], cwd=ROOT, text=True
    ).splitlines()
    cache = {}
    ...
    def advance(match):
        ...
        if not (
            re.fullmatch(r"[0-9a-f]{40}", ref) or re.fullmatch(r"v?\d+\.\d+\.\d+", ref)
        ):
        ...
        if key not in cache:
            if key[2] == "sha":
                branch = (
                    ...
                    else api(f"repos/{owner}/{repo}")["default_branch"]
                )
                ...
                stable = [
                    tag for tag in stable if re.fullmatch(r"v?\d+\.\d+\.\d+", tag)
                ]
                if not stable:
                    stable = [
                        tag["name"]
                        for tag in api(f"repos/{owner}/{repo}/tags?per_page=100")
                        if re.fullmatch(r"v?\d+\.\d+\.\d+", tag["name"])
                    ]
                cache[key] = max(
                    [ref, *stable],
                    key=lambda tag: tuple(map(int, tag.lstrip("v").split("."))),
                )
