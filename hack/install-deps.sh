...
if ! command -v darwin-rebuild &>/dev/null; then
    ...
    cd /tmp/nix-darwin-bootstrap || { log_critical "Failed to change to bootstrap directory"; exit 1; }
    ...
    cd - >/dev/null || { log_critical "Failed to change back to original directory"; exit 1; }
