    ...
    def test_updates_fixed_refs_and_nested_locks_once(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "flake.nix").write_text(
                '"github:owner/tool/v1.0.0?dir=extras" "github:owner/tool/main"'
            )
            (root / "extras/flake.nix").write_text(
                '"github:owner/tool/' + "a" * 40 + '"'
            )
            calls = []
            ...
            self.assertEqual(
                ...
                '"github:owner/tool/v2.0.0?dir=extras" "github:owner/tool/main"',
            )
            self.assertIn("b" * 40, (root / "extras/flake.nix").read_text())
            self.assertEqual(
                [entry[1]["cwd"] for entry in calls], [root, root / "extras"]
            )
            self.assertTrue(
                all(
                    entry[0] == ["nix", "flake", "update"] and entry[1]["check"]
                    for entry in calls
                )
            )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def output(argv, **kwargs):
                ...
                if "/releases?" in argv[-1]:
                    return "[]"
                return json.dumps(
                    [{"name": "v1.9.0"}, {"name": "v3.0.0-rc1"}, {"name": "v2.1.0"}]
                )
